...
# Import report generation from Evaluation module
from ConversationalAI_Assignment_2_Group_122_Part2_Evaluation import (
...
def load_ablation():
    ...
    # Default sample data
    return {
        "Dense-only": {"mrr": 0.65, "rouge_l_f1": 0.35, "bert_score_f1": 0.72, "top1_accuracy": 0.55, "top3_accuracy": 0.72},
        "Sparse-only": {"mrr": 0.60, "rouge_l_f1": 0.32, "bert_score_f1": 0.68, "top1_accuracy": 0.50, "top3_accuracy": 0.68},
        "Hybrid (k=30)": {"mrr": 0.73, "rouge_l_f1": 0.40, "bert_score_f1": 0.76, "top1_accuracy": 0.62, "top3_accuracy": 0.80},
        "Hybrid (k=60)": {"mrr": 0.76, "rouge_l_f1": 0.42, "bert_score_f1": 0.78, "top1_accuracy": 0.65, "top3_accuracy": 0.83},
        "Hybrid (k=100)": {"mrr": 0.74, "rouge_l_f1": 0.41, "bert_score_f1": 0.77, "top1_accuracy": 0.63, "top3_accuracy": 0.81}
    }
...
def generate_sample_data():
    ...
    questions = [
        {"id": f"q{i:03d}", "question": f"Sample question {i}?",
         "ground_truth": f"Sample answer {i}.", "category": ["factual", "comparative", "inferential", "multi_hop"][i % 4],
         "source_url": f"https://en.wikipedia.org/wiki/Topic_{i}"}
        for i in range(1, 101)
    ]

    results = pd.DataFrame({
        "question_id": [f"q{i:03d}" for i in range(1, 101)],
        "category": [["factual", "comparative", "inferential", "multi_hop"][i % 4] for i in range(100)],
        "question": [f"Sample question {i}?" for i in range(1, 101)],
        "ground_truth": [f"Sample answer {i}." for i in range(1, 101)],
        "generated_answer": [f"Generated answer {i}." for i in range(1, 101)],
        "reciprocal_rank": np.random.uniform(0.2, 1.0, 100),
        "rouge_l_f1": np.random.uniform(0.2, 0.7, 100),
        "bert_score_f1": np.random.uniform(0.5, 0.9, 100)
    })
...
def main():
    # Sidebar
    st.sidebar.image("https://upload.wikimedia.org/wikipedia/commons/thumb/8/8a/Banana-Single.jpg/100px-Banana-Single.jpg", width=50)
    ...
    if st.sidebar.button("📄 Generate HTML Report", help="Generate comprehensive HTML evaluation report"):
        if results_exists and ablation_exists:
            try:
                with st.spinner("Generating HTML report..."):
                    # Load all necessary data
                    with open("evaluation_questions.json", "r", encoding="utf-8") as f:
                        questions_data = json.load(f)
                    results_data = pd.read_csv("evaluation_results.csv")
                    with open("ablation_results.json", "r", encoding="utf-8") as f:
                        ablation_data_loaded = json.load(f)

                    # Prepare metrics structure for report
                    metrics_for_report = {
                        'mrr': {
                            'mrr': results_data['reciprocal_rank'].mean(),
                            'interpretation': 'Good' if results_data['reciprocal_rank'].mean() > 0.6 else 'Moderate',
                            'reciprocal_ranks': results_data['reciprocal_rank'].tolist(),
                            'statistics': {
                                'top1_accuracy': (results_data['reciprocal_rank'] == 1.0).mean(),
                                'top3_accuracy': (results_data['reciprocal_rank'] >= 0.33).mean()
                            }
                        },
                        'rouge_l': {
                            'mean_f1': results_data['rouge_l_f1'].mean(),
                            'scores': [{'f1': f} for f in results_data['rouge_l_f1'].tolist()],
                            'justification': "ROUGE-L measures longest common subsequence between generated and reference answers."
                        },
                        'bert_score': {
                            'mean_f1': results_data['bert_score_f1'].mean(),
                            'scores': [{'f1': f} for f in results_data['bert_score_f1'].tolist()],
                            'justification': "BERTScore uses contextual BERT embeddings to measure semantic similarity."
                        }
                    }

                    # Prepare results list
                    results_list = results_data.to_dict('records')

                    # Prepare error analysis
                    error_analysis = {
                        'retrieval_failures': [{'question_id': r['question_id'], 'category': r.get('category', 'unknown'), 'question': r.get('question', '')} 
                                               for r in results_list if r.get('reciprocal_rank', 0) == 0][:5],
                        'generation_failures': [{'question_id': r['question_id'], 'reference': r.get('ground_truth', '')[:100], 'generated': r.get('generated_answer', '')[:100]} 
                                                for r in results_list if r.get('rouge_l_f1', 1) < 0.2][:5],
                        'category_performance': {},
                        'summary': {
                            'total_questions': len(results_list),
                            'retrieval_failure_count': sum(1 for r in results_list if r.get('reciprocal_rank', 0) == 0),
                            'generation_failure_count': sum(1 for r in results_list if r.get('rouge_l_f1', 1) < 0.2)
                        }
                    }

                    # Generate the report
                    generate_html_report(results_list, metrics_for_report, ablation_data_loaded, error_analysis, "evaluation_report.html")
                    st.sidebar.success("✅ Report generated: evaluation_report.html")
            except Exception as e:
                st.sidebar.error(f"Error: {str(e)}")
        else:
            st.sidebar.warning("⚠️ Need both results and ablation data to generate report")

    # Recalculate Metrics button
    if st.sidebar.button("🔢 Recalculate Metrics", help="Recalculate all metrics from current data"):
        if results_exists:
            try:
                with st.spinner("Recalculating metrics..."):
                    results_data = pd.read_csv("evaluation_results.csv")

                    generated = results_data['generated_answer'].fillna('').tolist()
                    references = results_data['ground_truth'].fillna('').tolist()

                    # Calculate ROUGE-L
                    rouge_results = calculate_rouge_l(generated, references)

                    # Calculate BERTScore
                    bert_results = calculate_bert_score(generated, references)

                    # Update CSV with new scores
                    results_data['rouge_l_f1'] = [s['f1'] for s in rouge_results['scores']]
                    results_data['bert_score_f1'] = [s['f1'] for s in bert_results['scores']]
                    results_data.to_csv("evaluation_results.csv", index=False)

                    st.sidebar.success(f"✅ Metrics updated!\nROUGE-L: {rouge_results['mean_f1']:.4f}\nBERTScore: {bert_results['mean_f1']:.4f}")
                    st.cache_data.clear()
                    st.rerun()
            except Exception as e:
                st.sidebar.error(f"Error: {str(e)}")
        else:
            st.sidebar.warning("⚠️ No evaluation results found")
    ...
    if not questions and results_df.empty:
        st.warning("⚠️ No data files found. Displaying sample data.")
        questions, results_df = generate_sample_data()
